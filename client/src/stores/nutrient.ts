...
export const useNutrientStore = defineStore("nutrient", {
  ...
  actions: {
    getNutrients() {
      ...
      fetch("http://localhost/api/nutrients")
        .then((res) => res.json())
        .then((json) => {
    ...
    getMeals() {
      fetch("http://localhost/api/meals")
        .then((res) => res.json())
    ...
    toggleFavourite(nutrient: Nutrient) {
      ...
      const url = `http://localhost/api/nutrients/${nutrient.id}`;
      const options = {
        method: "PUT",
        headers: {
    ...
    updateFavouriteOrder() {
      ...
      const url = `http://localhost/api/favourites/nutrients`;
      const options = {
        method: "POST",
        headers: {
          Accept: "application/json",
